...
macro_rules! implement_buildable_for_lambda_with_type {
    ($Builder:ty) => {
        ...
        impl<T> Buildable<$Builder> for T where T: Fn(&mut $Builder) -> &mut $Builder {
            fn on_build<'a>(
                &self,
                builder: &'a mut $Builder,
            ) -> &'a mut $Builder {
    ...
    T: Buildable<serenity::CreateActionRow>
{
    fn on_build<'a>(
        ...
        builder.create_action_row(|builder| <Self as Buildable<serenity::CreateActionRow>>::on_build(&self, builder))
    }
}
